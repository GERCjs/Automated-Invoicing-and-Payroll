...
class EmployeeDashboardTests(TestCase):
    ...
    def test_hr_can_access_employee_dashboard_with_kpis_and_charts(self):
        ...
        response = self.client.get(reverse("employee-dashboard"), data={"month": self.report_month}, follow=True)

        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, f'{reverse("payroll-dashboard")}?month={self.report_month}')
